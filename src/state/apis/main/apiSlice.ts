...
export const mainApiSlice = createApi({
	...
	endpoints: builder => ({
		...
		getBalanceUpdate: builder.query({
			query: () => ({ url: `/enduser/get-user-balance`, method: 'post' }),
		}),
		getStackDetails: builder.query({
			query: () => ({ url: `/enduser/get-stake-button`, method: 'post' }),
		}),
		getAllCasionType: builder.query({
			query: () => ({ url: `/casino/all-casino-types`, method: 'post' }),
		}),
		getJwtToken: builder.query({
			query: () => ({ url: `/util/validate-jwt-token`, method: 'post' }),
