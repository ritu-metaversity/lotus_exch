import { Box } from "@mui/material";
import "./style.scss";
import PlayArrowRoundedIcon from "@mui/icons-material/PlayArrowRounded";
import { useNavigate } from "react-router-dom";


const Cricket = () => {
  const nav = useNavigate();
  const handleNav = () => {
    nav("/sport/4/e/33450249");
  };

  return (
    <section className="inplay-upcoming-container app-content ignore-content-spacing">
      <section className="section in-play">
        <Box>
          <div
            className="section-title in-play"
            style={{
              padding: "3px 0",
            }}
          >
            <div
              style={{
                display: "flex",
                alignItems: "center",
              }}
            >
              <PlayArrowRoundedIcon
                sx={{
                  fontSize: "30px",
                  color: "#257b23",
                }}
              />

              <p
                className="label"
                style={{
                  margin: "0",
                }}
              >
                Live
              </p>
            </div>
          </div>
        </Box>
        <Box _nghost-bfg-c188="">
          <div className="sport-events-group-container">
            <div className="sport-group-header first">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">The Hundred - Womens</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2106445">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <Box _nghost-bfg-c169="">
                                  <div _ngcontent-bfg-c169="" className="label">
                                    In Play
                                  </div>
                                </Box>
                              </div>
                            </div>
                            <div className="competitors" onClick={handleNav}>
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">
                                    Who will win The Hundred Womens 2024?
                                  </div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="more full-width">
                              <div className="inner">See more markets</div>
                              <i className="icon icon-angle-right" />
                            </div>

                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container line-market">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button disabled back"></div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button disabled lay"></div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">Test Matches</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2147426">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <Box _nghost-bfg-c169="">
                                  <div _ngcontent-bfg-c169="" className="label">
                                    In Play
                                  </div>
                                </Box>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Ireland</div>
                                </div>
                                <div className="line">
                                  <div className="name">Zimbabwe</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">48</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled back"></div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled back"></div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">54</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled lay"></div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled lay"></div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">
                  Womens International Twenty20 Matches
                </span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info ">
                                <div className="line">3 Ov</div>
                                {/* <div className="line">3</div> */}
                                <div className="line">18/3</div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">India Women</div>
                                </div>
                                <div className="line batting">
                                  <div className="name">Bangladesh Women</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">1.05</div>

                                      <Box>
                                        <div className="market-volume">
                                          500,000+
                                        </div>
                                      </Box>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">17.5</div>

                                      <Box>
                                        <div className="market-volume">
                                          500,000+
                                        </div>
                                      </Box>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">1.06</div>

                                      <Box>
                                        <div className="market-volume">
                                          500,000+
                                        </div>
                                      </Box>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">18.5</div>

                                      <Box>
                                        <div className="market-volume">
                                          26,674
                                        </div>
                                      </Box>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">Tamil Nadu Premier League</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-1.230993791-5.FY">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <Box _nghost-bfg-c169="">
                                  <div _ngcontent-bfg-c169="" className="label">
                                    In Play
                                  </div>
                                </Box>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Salem Spartans</div>
                                </div>
                                <div className="line">
                                  <div className="name">Lyca Kovai Kings</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="more full-width">
                              <div className="inner">See more markets</div>
                              <i className="icon icon-angle-right" />
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-1.231004036-5.FY">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <Box _nghost-bfg-c169="">
                                  <div _ngcontent-bfg-c169="" className="label">
                                    In Play
                                  </div>
                                </Box>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Tiruppur Tamizhans</div>
                                </div>
                                <div className="line">
                                  <div className="name">
                                    Ruby Trichy Warriors
                                  </div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="more full-width">
                              <div className="inner">See more markets</div>
                              <i className="icon icon-angle-right" />
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>
          </div>
        </Box>
      </section>

      <section className="section in-play">
        <Box>
          <div className="section-title upcoming">
            <div>
              <span className="label">Upcoming</span>
            </div>
          </div>
        </Box>
        <Box _nghost-bfg-c188="">
          <div className="sport-events-group-container">
            <div className="sport-group-header first">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">The Hundred</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2106447">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    23/07/2024 23:00
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">
                                    Who will win The Hundred 2024?
                                  </div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="more full-width">
                              <div className="inner">See more markets</div>
                              <i className="icon icon-angle-right" />
                            </div>

                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container line-market">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button disabled back"></div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button disabled lay"></div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2148407">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 23:05
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">
                                    Northern Superchargers
                                  </div>
                                </div>
                                <div className="line">
                                  <div className="name">Trent Rockets</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">117</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">79</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">127</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">85</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2151076">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Tomorrow 23:05
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">London Spirit</div>
                                </div>
                                <div className="line">
                                  <div className="name">Birmingham Phoenix</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">77</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">114</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">87</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">130</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2157604">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    28/07/2024 23:05
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Welsh Fire</div>
                                </div>
                                <div className="line">
                                  <div className="name">Oval Invincibles</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">126</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">72</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">139</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">79</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">Metro Bank One Day Cup</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2151049">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 15:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Hampshire</div>
                                </div>
                                <div className="line">
                                  <div className="name">Northamptonshire</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">80</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">106</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">94</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">125</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2157285">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 15:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Nottinghamshire</div>
                                </div>
                                <div className="line">
                                  <div className="name">Sussex</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">74</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">113</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">88</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">136</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2157286">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 15:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Leicestershire</div>
                                </div>
                                <div className="line">
                                  <div className="name">Warwickshire</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">72</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">119</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">84</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">139</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2157284">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 18:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Somerset</div>
                                </div>
                                <div className="line">
                                  <div className="name">Kent</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">82</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">104</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">96</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">122</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2157872">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Tomorrow 15:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Derbyshire</div>
                                </div>
                                <div className="line">
                                  <div className="name">Middlesex</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">80</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">96</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">104</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">120</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">Test Matches</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2147810">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 15:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">England</div>
                                </div>
                                <div className="line">
                                  <div className="name">West Indies</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">17</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled back"></div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled back"></div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">20</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled lay"></div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled lay"></div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">
                  ICC Cricket World Cup League 2
                </span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2154294">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 15:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Scotland</div>
                                </div>
                                <div className="line">
                                  <div className="name">Namibia</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">47</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">178</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">56</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">213</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">
                  Womens International Twenty20 Matches
                </span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2155465">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 19:00
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Sri Lanka Women</div>
                                </div>
                                <div className="line">
                                  <div className="name">Pakistan Women</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">63</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">149</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">67</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">159</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">Tamil Nadu Premier League</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2150433">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 19:15
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Dindigul Dragons</div>
                                </div>
                                <div className="line">
                                  <div className="name">Madurai Panthers</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">39</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">196</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">51</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">257</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">The Hundred - Womens</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2148204">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 19:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">
                                    Northern Superchargers Women
                                  </div>
                                </div>
                                <div className="line">
                                  <div className="name">
                                    Trent Rockets Women
                                  </div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">107</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">79</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">127</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">93</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-1.231047131">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Tomorrow 19:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">
                                    London Spirit Women
                                  </div>
                                </div>
                                <div className="line">
                                  <div className="name">
                                    Birmingham Phoenix Women
                                  </div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button disabled back">
                                      <div className="price">1.51</div>

                                      <Box>
                                        <div className="market-volume">
                                          161,745
                                        </div>
                                      </Box>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled back">
                                      <div className="price">2.10</div>

                                      <Box>
                                        <div className="market-volume">
                                          39,159
                                        </div>
                                      </Box>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button disabled lay">
                                      <div className="price">1.90</div>

                                      <Box>
                                        <div className="market-volume">
                                          43,699
                                        </div>
                                      </Box>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button disabled lay">
                                      <div className="price">2.98</div>

                                      <Box>
                                        <div className="market-volume">
                                          129,396
                                        </div>
                                      </Box>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2157365">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    28/07/2024 19:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Welsh Fire Women</div>
                                </div>
                                <div className="line">
                                  <div className="name">
                                    Oval Invincibles Women
                                  </div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">125</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">72</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">139</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">80</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">Global T20 Canada</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2149258">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Today 20:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">
                                    Mississauga Bangla Tigers
                                  </div>
                                </div>
                                <div className="line">
                                  <div className="name">Montreal Tigers</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">76</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">111</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">90</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">132</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="event-item">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2149259">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Tomorrow 01:30
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Brampton Wolves</div>
                                </div>
                                <div className="line">
                                  <div className="name">Surrey Jaguars</div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">119</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">72</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">139</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">84</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">Major League Cricket</span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2157953">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Tomorrow 06:00
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Texas Super Kings</div>
                                </div>
                                <div className="line">
                                  <div className="name">
                                    San Francisco Unicorns
                                  </div>
                                </div>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">78</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">108</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">92</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">129</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>

            <div className="sport-group-header">
              <div className="group-title">
                <span className="title-item">Cricket</span>
                <span className="title-item">
                  International Twenty20 Matches
                </span>
              </div>
              <div className="group-odds-title">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
              <div className="group-odds-title hidden-on-short-screen">
                <div>1</div>
                <div>X</div>
                <div>2</div>
              </div>
            </div>
            <div className="event-item first">
              <Box>
                <div>
                  <div>
                    <Box>
                      <Box _nghost-bfg-c164="">
                        <div className="market" id="selection-9.2150406">
                          <Box className="market-title">
                            <div className="info-box">
                              <div className="info">
                                <div className="line date">
                                  <span className="date-info__item date-info__item--open-date">
                                    Tomorrow 19:00
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="competitors bet-builder-exists">
                              <div className="width-wrapper">
                                <div className="line">
                                  <div className="name">Sri Lanka</div>
                                </div>
                                <div className="line">
                                  <div className="name">India</div>
                                </div>
                              </div>
                              <div>
                                <button className="icon-genie-button">
                                  <i className="icon-genie-stroke" />
                                </button>
                              </div>
                            </div>
                          </Box>

                          <div className="odds market-odds custom-width">
                            <div className="odds-wrap scrollable">
                              <Box _nghost-bfg-c163="">
                                <div className="market-odds__container">
                                  <Box className="not-selected scrollable">
                                    <div className="bet-button back">
                                      <div className="price">243</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled back">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button back">
                                      <div className="price">38</div>
                                    </div>
                                  </Box>

                                  <Box className="not-selected scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">264</div>
                                    </div>
                                  </Box>

                                  <Box>
                                    <div className="bet-button has-no-runner disabled lay">
                                      -
                                    </div>
                                  </Box>

                                  <Box className="not-selected more-odds scrollable">
                                    <div className="bet-button lay">
                                      <div className="price">41</div>
                                    </div>
                                  </Box>
                                </div>
                              </Box>
                            </div>
                          </div>
                        </div>
                      </Box>
                    </Box>
                  </div>
                </div>
              </Box>
            </div>
          </div>
        </Box>
      </section>
    </section>
  );
};

export default Cricket;
