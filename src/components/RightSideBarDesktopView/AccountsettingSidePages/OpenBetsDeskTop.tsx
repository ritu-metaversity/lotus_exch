...
const OpenBetsDeskTop = () => {
    ...
    const [trigerBetHistory, { data: bethistorData }] = useBetHistoryMutation(undefined)
    ...
    useEffect(() => {
        if (bethistorData?.data && bethistorData?.data?.dataList) {
            setUnsettledbetmatchdata(bethistorData?.data && bethistorData?.data?.dataList)
        }
    ...
    useEffect(() => {
        ...
        if (unSettledBet?.data && unSettledBet?.data?.dataList) {
            setUnsettledbetmatchdata(unSettledBet?.data && unSettledBet?.data?.dataList)
        }
    }, [unSettledBet])
    console.log(unsettledbetmatchdata, "unsettledbetmatchdata");

    const StartDateValue = ({ date, dateString }: any) => {
        console.log(date, dateString, "dffssdfsdfsd");

        setStartDate(dateString);
    };

    const EndDateValue = ({ date, dateString }: any) => {
        console.log(date, dateString, "dffssdfsdfsd");
    ...
    useEffect(() => {
        let data = { betType: 1, index: 0, noOfRecords: 5, sportType: 1, isDeleted: "false" };
    ...
    const handleSeleteGame = (vl: any) => {
        if (vl === "Sport") {
            ...
            let data = { betType: 1, index: 0, noOfRecords: 5, sportType: 2, isDeleted: "false" };
            triger(data)
            //   dispatch(Postunsettleddddd(data));
        ...
        <div className="mid-pane">
            ...
            <div className="betHeading_main">
                ...
                <div className="betHeading_pastcurrnt">
                    <div className={`Past_Current ${current === true ? "selected" : ""}`} onClick={() => handleSelect("selected1")}>Current</div>
                    <div className={`Past_Current ${history === true ? "selected" : ""}`} onClick={() => handleSelect("selected2")}>Past</div>
                    {current === false ? "" :
                        <>
                            <div className={`Past_Current ${gameSportCasion === "Sport" ? "selecteddddd" : ""}`} onClick={() => handleSeleteGame("Sport")}>Sports</div>
                            <div className={`Past_Current ${gameSportCasion === "Casion" ? "selecteddddd" : ""}`} onClick={() => handleSeleteGame("Casion")}> casino</div>
                        </>}
                    {history === true ?
                ...
                {current === false ?
                    <div className="betHeading_datepicker">
                        <div className="mybets">
                            ...
                            <div className=" date_piker_from_to" style={{ marginLeft: "-18px" }}>
                            ...
                            <button className="MyOpenBets__page-menu-date-search-button" onClick={handleSearch}>
                                Search
                            </button>
