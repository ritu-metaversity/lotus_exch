...
export const createProfits = ({
	...
	if (betDetails?.stake != null && !rechange) {
		...
		if (betDetails?.marketName === 'Bookmaker') {
				...
				Odds: {
					...
					[betDetails?.allData?.marketId || 'really']: [
						...(fancyOdds?.Odds?.find(
							...
							if (
								element.selectionId ===
								betDetails?.SelectionIdForAll?.selectionId
							) {
								currentProfit.value =
			...
			// 		Fancy: [
			// 			profitFancy?.filter(
			// 				(i: ProfitInterface) => i.sid != betDetails?.SelectionIdForAll
			// 			),
			// 			{
			...
			// 				value:
			// 					(profitFancy?.find(
			// 						(item: ProfitInterface) =>
			// 							item.sid == betDetails.SelectionIdForAll
			// 					)?.value || 0) - (stake || 0),
			// 			},
			// 		],
			// 	}));
		...
		const newItem = {
			...
			Bookmaker: fancyOdds?.Bookmaker?.map((element: any) => {
				const pnlsBookmaker = pnl?.find(
					(pnl: any) => pnl?.marketId == element.mid
				);
