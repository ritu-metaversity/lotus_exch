import ArrowBackIcon from '@mui/icons-material/ArrowBack';
import "./GamePageInternational.css"
import { useLocation, useNavigate } from 'react-router-dom';
import { useEffect, useState } from 'react';
import axios from 'axios';
const GamePageInternational = () => {

    const TokenId = localStorage.getItem("token");
    const { state } = useLocation()
    const [gameLobbyUrl, setGameLobbyUrl] = useState("")
    const navigate = useNavigate();

    console.log(state, "sfjvdckdscn");

    useEffect(() => {
        const TokenGame = localStorage.getItem("GameToken");
        if (state?.gameCode === "Qtech") {
            let data = {
                playerId: "121212",
                currency: "INR",
                country: "IN",
                gender: "M",
                birthDate: "1986-01-01",
                lang: "en_IN",
                mode: "real",
                // device: window.innerWidth > 1024 ? "desktop" : "mobile",
                device: window.innerWidth > 1024 ? "desktop" : "mobile",
                returnUrl: `/home`,
                token: TokenGame,
                walletSessionId: TokenId
            }
            // https://api.playindia.app/api/qtech/gamelink
            axios.post("https://api.247idhub.com/api/qtech/gamelobby",
                data,
                {
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${TokenId}`,
                    },
                }
            )
                .then((response) => {
                    if (response) {
                        console.log(response?.data?.data?.url, "sdfsdfsdfsdfsdfsdfsd")
                        setGameLobbyUrl(response?.data?.data?.url)
                    }
                })
        } else {
            let data = {
                playerId: "121212",
                currency: "INR",
                country: "IN",
                gender: "M",
                gameName: state?.gameCode,
                birthDate: "1986-01-01",
                lang: "en_IN",
                mode: "real",
                device: window.innerWidth > 1024 ? "desktop" : "mobile",

                returnUrl: `${window.location.protocol}//${window.location.hostname}/Live-casino`,
                token: TokenGame,
                walletSessionId: TokenId
            }
            axios.post(
                "https://api.247idhub.com/api/qtech/gamelink",
                data,
                {
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${TokenId}`,
                    },
                }
            )
                .then((response) => {
                    if (response) {
                        console.log(response?.data?.data?.url, "sdfsdfsdfsdfsdfsdfsd")
                        setGameLobbyUrl(response?.data?.data?.url)
                    } else {

                    }
                })

        }

    }, [])

    const handleBack = () => {
        
        navigate("/home")
    }
    return (
        <div className="header_for_backbtn_casino">
            <div className="header_for_exitliveCasino" onClick={handleBack}>
                <ArrowBackIcon style={{ fontSize: "20px" }} />
                <span>Back to Home Page</span>
            </div>

            {/* <div> */}


            <iframe
                src={gameLobbyUrl}
                width="100%"
                style={{ height: "calc(100vh - 29px)" }}
                title="mobile"
                allowFullScreen={true}

            ></iframe>
            {/* </div> */}
        </div>
    )
}

export default GamePageInternational